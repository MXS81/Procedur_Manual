...
window.services = {
  ...
  /** �? HTML 头部解析声明编码（优�? http-equiv Content-Type 中的 charset�? */
  ...
  /** 供渲染进�? pdf.js 使用，避免依赖浏览器内嵌 PDF 插件 */
  ...
  /**
   * 导入文件夹时：若�? PHP 手册等多�? HTML 包，自动建议 entryFile，走 IframeManualReader（Node 解码 + srcdoc，避�? GBK 误标 UTF-8 乱码�?
   */
  suggestBundledHtmlEntry (dirPath) {
    ...
    // 仅在�? PHP �? chunk 目录时才�? Iframe；仅凭「页数多」会误判，导致无 chunklist 解析、左侧无目录且入口页可能加载失败
    if (hasChunkToc) {
  ...
  // ========== PDF Extraction ==========
  // 使用 pdf-parse v2（PDFParse 类）。旧版「函数式」API 已不可用，否则索引始终为空�?
  // 可选：安装 Poppler 并将 pdftotext 加入 PATH 作为兜底（https://github.com/oschwartz10612/poppler-windows/releases�?
  ...
  /**
   * 按页切分，供全文索引；onProgress?(currentPage, totalPages)
   */
  ...
  /**
   * 阅读器：较小 PDF �? base64 嵌入（uTools/WebView �? file:// 限制较多）；大文件用 file:// 省内存�?
   */
  ...
  // ========== CHM Decompilation ==========
  // 约定：打开/跳转 CHM 页时一律走 _resolveChmFilePath（大小写�?..、file://、目�?/index�?.hhc 实体解码），
  // 正文内链保留 # 锚点�? ChmReader 滚动�?.hhc 嵌套 </ul> 必须�? liStack 恢复�? <li>，否则三级以下目录错位�?

  /** 解压结果须含可读 HTML �? .hhc，避免把「有文件但残缺」的缓存当成�? */
  _chmExtractLooksUsable (extractDir) {
    try {
      const html = this.scanDir(extractDir, ['.html', '.htm'], { maxFiles: 2 })
  ...
  decompileChm (chmPath) {
    ...
    const run7z = () => {
      ...
      try {
        execFileSync(sevenZip, ['x', resolvedChm, outArg, '-y', '-bb0'], {
    ...
    const runHh = () => {
      try {
        execFileSync(hhPath, ['-decompile', extractDir, resolvedChm], {
    ...
    const hint = sevenZip
      ? '已尝�? 7-Zip �? hh.exe，解压结果仍无可读页靀��请确认 CHM 未损坏、非加密�?'
      : '未找�? 7-Zip 可执行文件（内置 public/tools/7za.exe 缺失时请重新执行 npm run build �? npm run bundle:7za）。亦可安装系统版 7-Zip 或设置环境变�? PM_SEVEN_ZIP�?'
  ...
  /**
   * CHM .hhc/.hhk �? Local 常见坑：file:/// 绝对路径、URL 编码、mk:/ms-its:�?
   * 路径一律规范成 / 分隔且无 #（锚点只在正文链接里由前端处理）�?
   */
  ...
  /** .hhc/.hhk �? PARAM �? VALUE 常带 &amp;�?&quot; 等，不解码会导致路径找不�? */
  _unescapeHhcAttr (s) {
    if (s == null || s === '') return ''
    return String(s)
  ...
  /**
   * 在解压目录内解析 CHM Local：支�? ../ 相对路径、段级大小写不敏感、末段为目录时尝�? index.html�?
   */
  ...
  /** 把目录里解析到的 Local 统一成相对解压目录、且与磁盘真实路径一致的小写�? */
  ...
  /**
   * 将外�? script / stylesheet 内联，避�? srcdoc+UTF-8 �? GBK �? .js/.css 被浏览器误读
   */
  ...
  /**
   * 读取解压根下的单�? HTML：内�?/用户导入�? .chm 解压目录，或用户导入的离�? HTML 整站目录�?
   * 路径必须�? _resolveChmFilePath�?..、大小写、目�? index）；不要在其�? API 里自�? join 后读文件�?
   */
  ...
  /**
   * 用户导入的离�? HTML 包：�? CHM 完全同一套读页与路径安全逻辑（见 getChmPageSrcdoc）�?
   */
  ...
  /**
   * 与前�? searchModes.compileSearchMatcher 行为一致（CHM/目录 HTML 全文扫描�?
  ...
  _parseHhcSitemap (html) {
    const items = []
    const stack = [items]
    /** 嵌套 <ul> 关闭后必须回到外�? <li>，否则三级以下目录的 PARAM 会挂到错误节点或丢失 */
    const liStack = []
