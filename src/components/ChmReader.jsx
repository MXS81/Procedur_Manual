...
function TocItem ({ node, activePath, onSelect, depth = 0 }) {
  ...
  return (
    <li className="chm-toc-item">
      ...
      >
        {hasChildren && (
          <span className={'chm-toc-arrow' + (expanded ? ' expanded' : '')}>&#9654;</span>
        )}
        {!hasChildren && <span className="chm-toc-dot" />}
        <span className="chm-toc-text">{node.name}</span>
      </div>
      {hasChildren && expanded && (
        <ul className="chm-toc-children">
